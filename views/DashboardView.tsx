...
const DashboardView: React.FC = () => {
  ...
  // Lógica simples para gráfico mensal (últimos 6 meses)
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-10">
        ...
        <p className="text-gray-500 font-medium">Resumo operacional do sistema.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <Card title="Pagamentos Pendentes" value={`R$ ${stats.pagamentosPendentes.toFixed(2)}`} icon="💰" color="red" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="flex items-end justify-between h-48 px-2">
            {monthlyStats.map((m, i) => (
              <div key={i} className="flex flex-col items-center flex-1">
                <div 
                  className="w-full max-w-[40px] bg-blue-500 rounded-t-md transition-all duration-1000"
                ...
                <p className="text-[9px] font-bold text-blue-600">{m.total.toFixed(0)}m</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold text-gray-800 mb-6">Status dos Riscos</h3>
                  ...
                  <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
                    <div 
                      className={`h-full transition-all duration-1000 ${
                        ...
                        status === 'Aguardando Risco' ? 'bg-yellow-500' : 'bg-gray-400'
                      }`}
                      style={{ width: `${percent}%` }}
                    ></div>
                  </div>
...
const Card: React.FC<{ title: string, value: string | number, icon: string, color: string }> = ({ title, value, icon, color }) => {
  ...
  return (
    <div className={`bg-white p-6 rounded-2xl shadow-sm border ${colors[color]} transition-transform hover:scale-[1.02]`}>
